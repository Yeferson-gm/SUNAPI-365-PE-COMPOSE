...
final class CatalogsResource extends AbstractResource
{
    ...
    {
        return $this->request($this->versionedPath($environment, 'system/info'));
    }
    ...
    {
        return $this->request($this->versionedPath($environment, 'dispatch-guides/catalogs/transfer-reasons'));
    }

    public function getDispatchGuideTransportModes(string $environment): array
    {
        return $this->request($this->versionedPath($environment, 'dispatch-guides/catalogs/transport-modes'));
    }

    public function getVoidedReasons(string $environment): array
    {
        return $this->request($this->versionedPath($environment, 'voided-documents/reasons'));
    }

    public function listVoidedAvailableDocuments(string $environment, array $query = []): array
    {
        return $this->request($this->versionedPath($environment, 'voided-documents/available-documents'), [
            'query' => $query,
        ]);
    }
}
